![Static Badge](https://img.shields.io/badge/kubernetes-blue) ![Static Badge](https://img.shields.io/badge/output-green)

# Kubernetes Notes

## 一、概述

- [Kubernetes 简介](./kubernetes-简介.md)
- [Kubernetes 架构](./kubernetes-架构.md)
- [Kubernetes 对象](./kubernetes-对象.md)
  - [标识符 Name&ID](./kubernetes-标识符.md)
  - [标签 Labels](./kubernetes-标签.md)
  - [注解 Annotations](./kubernetes-注解.md)
  - [命名空间 Namespaces](./kubernetes-命名空间.md)
  - [标签选择器 Label Selectors](./kubernetes-标签选择器.md)
  - [字段选择器 Field Selectors](./kubernetes-字段选择器.md)
- [Kubernetes Pod](./kubernetes-Pod.md)